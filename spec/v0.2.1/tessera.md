# TESSERA v0.2.1 — Protocol Specification
...
**Supersedes.** `spec/v0.2/tessera.md` (retained); `spec/v0.1/tessera.md` (retained as historical artifact).
...
v0.2.1 amends v0.2 with one additive sub-field: `composition_analysis.paste_events`, an optional auxiliary array recording the source classification of each paste event during a session. The field carries no enforcement weight on its own — bucket totals remain the source of truth for authorship percentages — but it enables verifiers to distinguish AI-tool pastes (cheating vector) from research quotations (legitimate) from internal own-work pastes (likely human authorship continuity). See §13.6.
...
v0.2.1 is **strictly additive** over v0.2 (and transitively over v0.1): every v0.2 Tessera is a valid v0.2.1 Tessera, and every v0.1 Tessera is a valid v0.2.1 Tessera. v0.2.1 verifiers MUST accept Tesserae with or without `paste_events`. See §15 "Version Compatibility".
...
function verify(tessera):
  # 1. Schema validation
  if not matches_schema(tessera, "tessera/v0.2.1"):
  ...
  if tessera.composition_analysis is not None:
    ...
    # 8a. Paste event invariants (v0.2.1, when present)
    if tessera.composition_analysis.paste_events is not None:
      if not check_paste_event_invariants(
          tessera.composition_analysis.paste_events,
          tessera.composition_analysis.total_session_ms):
        return PASTE_EVENT_INVARIANT_VIOLATION
...
New for v0.2.1 (`paste_events` auxiliary exercise — also under `test-vectors/v0.2.1/`):
- `v0.2.1/paste_forensics_ai_heavy.json` — VALID, 45-min session, 4 paste events: 2 ChatGPT (`ai_tool`), 1 Wikipedia (`research_quotation`), 1 Apple Notes (`internal_authored`); bucket distribution skewed toward `ai_assisted_ms`.
- `v0.2.1/paste_forensics_research_heavy.json` — VALID, 90-min academic session, 6 paste events: 5 arxiv/Wikipedia (`research_quotation`), 1 own-draft (`internal_authored`); high `human_active_ms`, zero `ai_assisted_ms`.
- `v0.2.1/paste_forensics_empty.json` — VALID, 30-min pure-keyboard session with `paste_events: []` (exercises the empty-array path).
...
## 13. COMPOSITION ANALYSIS (v0.2, extended in v0.2.1)
...
v0.2.1 adds one optional auxiliary sub-field — `paste_events` (§13.6) — that records the source classification of each paste event. The auxiliary field is **forensic, not enforcement**: it provides evidence a verifier can inspect, but the bucket totals (§13.2) remain the canonical authorship measurement. A v0.2.1 verifier that lacks paste-classifier support MUST still accept the Tessera; only the bucket invariants gate validity.
...
### 13.6 Paste forensics — `paste_events` (auxiliary, v0.2.1)

`paste_events` is an **optional** array within `composition_analysis`. When present, it contains one entry per paste event observed during the session, classifying each by source so verifiers can distinguish AI-tool pastes, research quotations, and own-work continuity.

This field is **forensic, not enforcement.** The bucket totals (§13.2) are the canonical authorship measurement; `paste_events` provides evidence a verifier can inspect to corroborate or contextualize them. A Tessera with bucket invariants satisfied is valid regardless of whether `paste_events` is present.
...
#### 13.6.2 Field semantics

- **`timestamp_ms`** (integer, required): Offset from session start, in milliseconds. MUST satisfy `0 ≤ timestamp_ms ≤ total_session_ms`.
- **`size_chars`** (integer, required, > 0): Number of characters in the pasted content. Used for proportionality signals only; the content itself is never recorded.
- **`source_classification`** (string, required, closed enum): One of:
  - `"ai_tool"` — Known AI chat interface or AI assistant process. Contributes to `ai_assisted_ms`.
  - `"research_quotation"` — Wikipedia, arxiv, news, documentation. Contributes to `paste_inserted_ms`.
  - `"internal_authored"` — User's own internal authoring environment (e.g., own notes app, own draft document). Contributes to `paste_inserted_ms`.
  - `"messaging"` — Slack, Discord, iMessage, email. Contributes to `paste_inserted_ms`.
  - `"code_repository"` — GitHub, GitLab, Stack Overflow. Contributes to `paste_inserted_ms`.
  - `"unknown"` — Source not classifiable. Contributes to `paste_inserted_ms`.
- **`source_evidence`** (string, required, 1–256 chars): Compact identifier with format `{type}:{specifier}`. Examples: `"chrome:chat.openai.com"`, `"app:com.openai.chat"`, `"process:com.anthropic.claudefordesktop"`, `"app:com.apple.Notes"`, `"unknown:unknown"`. Evidence is descriptive — verifiers MUST NOT treat it as cryptographic proof.
- **`confidence`** (enum, required): `"high" | "medium" | "low"`. Reflects the classifier's confidence in this specific event's classification.
- **`contributed_to_bucket`** (enum, required): The bucket name this event's time contributed to. MUST be one of: `human_active_ms`, `ai_assisted_ms`, `voice_authored_ms`, `paste_inserted_ms`, `context_review_ms`, `idle_ms`.

#### 13.6.3 Invariants

A valid v0.2.1 Tessera with `paste_events` present MUST satisfy ALL of the following:

1. **Optionality.** `paste_events` MAY be empty (`[]`) or omitted entirely; both are valid.
2. **Ordering.** Events SHOULD be ordered by `timestamp_ms` ascending. Verifiers MUST NOT reject a Tessera for out-of-order events; they MAY sort for display.
3. **Bounds.** `timestamp_ms` MUST satisfy `0 ≤ value ≤ total_session_ms` for every event.
4. **Bucket reference.** `contributed_to_bucket` MUST be one of the six bucket names defined in §13.2.
5. **Classification enum.** `source_classification` MUST be one of the six closed-enum values in §13.6.2.
6. **Content privacy.** `paste_events` MUST NOT contain pasted content, content hashes, or any data that could leak the pasted text. Only the count (`size_chars`), source (`source_evidence`), and classifier verdict are recorded.

#### 13.6.4 Verification semantics

A v0.2.1 verifier MUST:

- Accept Tesserae without `paste_events` (capture-disabled, paste-classifier-disabled, or empty-array cases).
- When `paste_events` is present: validate invariants 1–6 (above).
- Reject with explicit error code `PASTE_EVENT_INVARIANT_VIOLATION` when any invariant fails.
- Surface paste forensics (count by classification, source-evidence list, suspicious-event flags) to the verifying party.

A v0.2.1 issuer MUST:

- Include `paste_events` only when the classifier observed and classified paste events.
- Use closed-enum values exactly as defined; arbitrary new classifications are forbidden until a future spec version extends the enum.
- Never include the pasted content or a hash thereof.

#### 13.6.5 Worked policy example

A university enforcing `academic_70_plus` (≥ 70% human) on submitted essays might additionally apply a "no AI-tool pastes" rule. Under v0.2.1 the verifier looks at `paste_events`, counts events with `source_classification: "ai_tool"`, and rejects if the count exceeds the policy threshold — even if the bucket-derived `human_pct` is above 70%. This is a verifier-side policy decision; the protocol provides the evidence, the institution sets the rule.
...
**v0.2.1 is strictly additive over v0.2; v0.2 is strictly additive over v0.1.**

- Every v0.1 Tessera is a valid v0.2 Tessera, and every v0.2 Tessera is a valid v0.2.1 Tessera. No prior field changes shape, type, or semantics.
- A v0.2.1 issuer MAY include `composition_analysis.paste_events`. A v0.2.1 issuer MAY omit it (or omit the entire `composition_analysis` field). All three modes are conformant.
- A v0.2.1 verifier MUST accept Tesserae with or without `paste_events`. When the field is present and any of its invariants fail, the verifier MUST reject with `PASTE_EVENT_INVARIANT_VIOLATION` (§13.6.4).
- A v0.2 verifier presented with a v0.2.1 Tessera will ignore the unknown `paste_events` array (per §12 "verifiers MUST ignore unknown fields") and verify the Tessera under v0.2 rules — composition_analysis bucket invariants still apply, paste forensics are silently dropped. A v0.1 verifier similarly ignores both `composition_analysis` and `paste_events`.
- The envelope `version` string is `"tessera/v0.2.1"` for new Tesserae issued under this spec. Tesserae with `version: "tessera/v0.2"` or `"tessera/v0.1"` remain valid indefinitely; issuers SHOULD migrate to `"tessera/v0.2.1"` once their tooling supports `paste_events`, but the migration is not required.
- The v0.1 and v0.2 specs (`spec/v0.1/tessera.md`, `spec/v0.2/tessera.md`) and their JSON Schemas are preserved unchanged. v0.1 and v0.2 test vectors are carried forward into `test-vectors/v0.2.1/` as a backward-compatibility check.
